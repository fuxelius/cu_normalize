...
int slice2arc_record(struct arc_record **arc_table, int *arc_len, struct mag_record **mag_table, int *mag_len, int chunk_size) {

    int arc_size = (chunk_size/WARP_SIZE) * WARP_SIZE; // approximately the same size as chunk_size
    *arc_len =  (*mag_len)/arc_size + 1;  //

    printf("arc_size=%i (%i*%i), arc_len=%i, mag_len=%i\n\n", arc_size, chunk_size/WARP_SIZE, WARP_SIZE, *arc_len, *mag_len);
